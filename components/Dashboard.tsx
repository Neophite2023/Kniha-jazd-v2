...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  if (!activeCar) {
    return (
      <div className="space-y-4 animate-in fade-in duration-500">
        <div className="bg-[#3f3f42] rounded-3xl p-8 border border-[#4d4d50]/50 shadow-sm text-center">
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500">
      <div className="bg-[#3f3f42] rounded-3xl p-5 border border-[#4d4d50]/50 shadow-sm relative overflow-hidden">
        {/* Car Selector */}
        <div className="absolute top-5 right-5 z-10">
          {settings.cars.length > 1 && (
            <select
              ...
              className="bg-[#38383b] text-zinc-100 text-[10px] font-bold uppercase tracking-wide py-1.5 px-3 rounded-full outline-none border-none cursor-pointer hover:bg-[#4d4d50] transition-colors appearance-none"
        ...
        <div className="absolute top-[64px] right-5 z-10">
          <button
            ...
            className="flex items-center gap-1.5 bg-[#38383b]/50 text-zinc-400 py-1.5 px-3 rounded-xl hover:text-green-400 hover:bg-[#38383b] transition-all border border-[#4d4d50]/50 active:scale-95"
              ...
              if (reminder.type === 'date' && reminder.targetDate) {
                ...
                return (
                  <div key={reminder.id} className="space-y-2">
                    ...
                    <div className="h-1.5 bg-[#38383b] rounded-full overflow-hidden">
                      <div
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center px-1">
          ...
          <button onClick={onViewAll} className="text-xs font-medium text-zinc-500 dark:text-zinc-500 hover:text-[#313134] dark:hover:text-white transition-colors">Všetky</button>
        </div>
          ...
          {recentTrips.length > 0 ? (
            recentTrips.map((trip, idx) => (
              <div
                ...
                className={`p-4 flex justify-between items-center active:bg-[#38383b] transition-all cursor-pointer select-none touch-manipulation ${idx !== recentTrips.length - 1 ? 'border-b border-[#4d4d50]/30' : ''}`}
                role="button"
                tabIndex={0}
              >
